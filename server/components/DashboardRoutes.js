...
const spotifyFetch = async (path, accessToken, options = {}) => {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Spotify ${res.status} on ${path}: ${errorText}`);
  }
...
router.post('/buffer', async (req, res) => {
  ...
  if (!current_z_sequence || current_z_sequence.length < 5 || current_z_sequence.length > 9) {
    return res.status(400).json({
      error: 'current_z_sequence must be an array of 5 to 9 z-vectors'
    });
  }
...
router.post('/playList', async (req, res) => {
  ...
  try {
    ...
    console.log(
      ...
      `seq_len=${current_z_sequence ? current_z_sequence.length : 0} — ` +
